"""Same as train_masked_reconstruction_freqinit.py (log-spaced frequency
init), but with freeze_freq=False: the frequency weight starts at the
same log-spaced init (avoiding the slow-discovery problem random init has
under spectral bias) but stays trainable, so gradient descent can still
concentrate capacity where the data's actual period distribution has
mass. The frozen variant clearly underperformed plain random init (probe
R^2 0.629 vs 0.835) -- this tests whether that was because freezing
specifically sacrificed useful flexibility, or whether log-spaced init
just isn't helpful here regardless of trainability.
...
CKPT_PATH = os.path.join(OUTDIR, 'mtand_masked_recon_freqinit_unfrozen.pt')
...
def build_model():
    ...
    encoder = MTANReconEncoder(input_dim=1, query=query, latent_dim=LATENT_DIM,
                                ...
                                learn_emb=True, freq_init='log_spaced', freeze_freq=False)
...
def main(time_budget_s=520):
    ...
    if epoch >= TOTAL_EPOCHS:
        ...
        with open(os.path.join(OUTDIR, 'masked_recon_freqinit_unfrozen_metrics.txt'), 'w') as f:
