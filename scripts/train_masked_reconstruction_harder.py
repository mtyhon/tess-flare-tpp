"""Masked reconstruction, pushed harder: same deterministic encoder +
NLL decoder as train_masked_reconstruction.py, but with the three fixes
recommended for insufficient periodicity learning:

1. Much longer training (target 600 epochs vs. the previous 75 -- an
   order of magnitude more, though still far short of the ~5000-epoch
   scale used in the reference notebook this project has been comparing
   against, which isn't practical to fully replicate here).
2. A cosine LR schedule over the full target horizon, instead of a flat
   lr=1e-3 throughout.
3. embed_time=128 (up from 64) for more time-embedding frequency capacity.
...
so a long training budget isn't vulnerable to any single call being
interrupted.
"""
import math
import os
...
CKPT_PATH = os.path.join(OUTDIR, 'mtand_masked_recon_harder.pt')
...
def main(time_budget_s=520):
    ...
    if epoch >= TOTAL_EPOCHS:
        ...
        with open(os.path.join(OUTDIR, 'masked_recon_harder_metrics.txt'), 'w') as f:
